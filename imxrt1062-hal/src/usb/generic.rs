/// This is the generic.rs module found in svd2rust. We're using it in
...
macro_rules! bit_writer {
    ($name:ident, $offset:expr) => {
        pub struct $name<'w>(&'w mut W);

        impl<'w> $name<'w> {
            #[inline(always)]
            pub fn bit(self, value: bool) -> &'w mut W {
                self.0.bits = (self.0.bits & !(1 << $offset)) | ((value as u32) << $offset);
                self.0
            }

            #[inline(always)]
            pub fn clear_bit(self) -> &'w mut W {
                self.bit(false)
            }

            #[inline(always)]
            pub fn set_bit(self) -> &'w mut W {
                self.bit(true)
            }
        }
    };
